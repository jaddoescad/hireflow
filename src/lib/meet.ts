...
async function syncSession(company: string, session: Interview, google: Google,
  ...
  if (!space || pending || setup === "pending" || setup === "failed") {
    try {
      ...
      // Only the organizer can pre-configure artifacts. Gemini notes stay off: no automated hiring assessments.
      await google(`https://meet.googleapis.com/v2/${space}?updateMask=config.artifactConfig.recordingConfig.autoRecordingGeneration,config.artifactConfig.smartNotesConfig.autoSmartNotesGeneration`, "PATCH", {
