...
import { useSave } from "./settings";

// Shows the outcome of Google's consent screen on the page that started it.
export function GoogleResult() {
  const result = typeof window === "undefined" ? null : new URLSearchParams(location.search).get("google");
...
export function Integrations({
  ...
  async function googleAction(action: "connect" | "sync" | "disconnect") {
    ...
    try {
      ...
      if (action === "connect") {
        window.location.assign((await post("/api/google/connect", { view: "integrations" })).url);
        return;
      }
    ...
    <>
      <header className="page-header">
        <div>
          ...
          <h1>Integrations</h1>
          <p>Email, calendar, phone and applicant sources for {data.company?.name}.</p>
        </div>
      </header>
